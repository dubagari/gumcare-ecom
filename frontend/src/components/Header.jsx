...
import { Heart, User, ShoppingCart, Search, ChevronDown, Home, Menu, LogOut } from "lucide-react";
    ...
    <header className="sticky top-0 z-50 bg-white/80 backdrop-blur-md">
      <nav className="p-3 border-b border-gray-200">
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          ...
          <Link to="/wishlist" className="p-2 text-gray-600 hover:bg-gray-100 rounded-full transition-colors relative group">
            <Heart size={20} className="group-hover:text-[#53C6E2] transition-colors" />
            <span className="absolute top-1 right-1 w-4 h-4 bg-[#53C6E2] text-white text-[10px] flex items-center justify-center rounded-full border-2 border-white">
              0
            </span>
          </Link>

          <Link to="/cart" className="p-2 text-gray-600 hover:bg-gray-100 rounded-full transition-colors relative group">
            <ShoppingCart size={20} className="group-hover:text-[#53C6E2] transition-colors" />
            {cartTotalQuantity > 0 && (
          ...
          {user ? (
            ...
            <Link to="/login" className="flex items-center gap-2 pl-2 sm:pl-4 border-l border-gray-200 group">
              ...
              <span className="text-sm font-bold text-gray-700 hidden sm:block group-hover:text-[#53C6E2]">Login</span>
            </Link>
          )}
        </div>
              </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center">
          <div className="flex items-center gap-8">
            ...
            <div className="hidden lg:flex items-center gap-6">
              {['Top Deals', 'Brand', 'New Arrivals', 'Best Sellers', 'Today\'s Offer'].map((item) => (
                <Link 
                  key={item} 
